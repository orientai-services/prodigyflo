...
| D8 | **83 mock extractions across 10 leads are sitting in production.** Production ran `EXTRACTION_PROVIDER=mock` from deploy until 2026-09-10 — a regex fixture that only matches a PDF text layer, so phone photos produced nothing and PDFs produced a handful of fields including a known `lease`/`loan` misread. Those rows are indistinguishable from real reads to anything downstream: the mirror would restate them, `/review` would ask a homeowner to confirm them, and delivery would carry them to ProdigyFlo. Options: reset them to `pending` so the real provider re-reads them (costs roughly $3 and is reversible), delete the test leads outright, or leave them and never trust a lead created before today. | These are all test leads today, which is the only reason it is a decision rather than an incident. | Dakota |
