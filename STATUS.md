...
### Product
- **T3 · The vision path is unproven.** B7 is closed (below) so scans now reach it, but it
  has 2 calls of production history averaging 21 of 25 nulls. The 57 print-to-PDF scans on
  lead `ecc46544` are the proving set: requeue them, drain, and read the `fields_found` per
  call. Until that number is measured, "the reader works on photographed paperwork" is a
  claim, not a fact.
