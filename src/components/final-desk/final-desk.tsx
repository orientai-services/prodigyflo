...
import { DocumentQuicklook } from './document-quicklook'
...
function Cell({ value }: { value: CaseCell }) {
  ...
  return <div className="cell"><label>{value.label}</label><div className={`v${c.kind === 'value' ? '' : ' m'}`}>
    {c.kind === 'value' ? c.display : c.kind === 'cannot_compute' ? t.cannotCompute : t.missing}
  </div>{(value.hint || value.unverified || c.kind === 'cannot_compute') && <small>{[value.unverified ? t.unverified : '', value.hint, c.kind === 'cannot_compute' ? `Needs ${c.missing.join(', ')}` : ''].filter(Boolean).join(' · ')}</small>}</div>
}
...
export function FinalDesk({ initial, view, clientId, query = {} }: { initial: FinalDeskPayload; view: DeskView; clientId?: string; query?: ClientQuery }) {
  ...
  if (view === 'board' && board) {
    ...
    content = mode === 'month' ? <><div className="banner">{board.unassignedCount} unassigned leads. Click a day for the hour view · click a name for the file.</div><div className="grid board-grid"><div className="card" ref={calendarRef}><div className="head"><h2>{board.title}</h2><button className="btn secondary" onClick={() => void selectDay(today)}>{t.today}</button></div><div className="week">{t.week.map(w => <span key={w}>{w}</span>)}</div><div className="days">{board.days.map(d => <div key={d.iso} className={`day${d.inMonth ? '' : ' mute'}${d.iso === today ? ' today' : ''}`} onClick={() => d.inMonth && void selectDay(d.iso)} onDragOver={e => e.preventDefault()} onDrop={e => d.inMonth && drop(e, d.iso)}>{d.inMonth && <><span className="num">{d.day}</span>{d.chips.filter(c => ['SCHEDULED','CONFIRMED'].includes(c.status)).map(c => chip(c))}</>}</div>)}</div></div>{unscheduled}</div></>
      : <><div className="banner">{dayChips.length ? `${dayChips.length} appointment${dayChips.length === 1 ? '' : 's'} this day. Click a block to open the file.` : 'No appointments this day. Drag someone from Unscheduled onto an hour.'}</div><div className="grid board-grid"><div className="card" ref={calendarRef}><div className="head"><h2>{dayTitle}</h2><div className="actions"><button className="btn secondary" onClick={() => shiftDay(-1)}>←</button><button className="btn secondary" onClick={() => void selectDay(today)}>{t.today}</button><button className="btn secondary" onClick={() => shiftDay(1)}>→</button><button className="btn" onClick={() => setMode('month')}>{t.month}</button></div></div><div className="agenda" style={{ padding: '0 0 18px' }}><div>{Array.from({ length: 12 }, (_, i) => <div className="agenda-hour" key={i}>{i + 8 < 12 ? `${i + 8} AM` : `${(i + 8) % 12 || 12} PM`}</div>)}</div><div className="agenda-track" style={{ minHeight: 768 }}>{Array.from({ length: 12 }, (_, i) => <div key={i} className="agenda-slot" onDragOver={e => e.preventDefault()} onDrop={e => drop(e, day, `${String(i + 8).padStart(2,'0')}:00`)} />)}{dayChips.map(c => chip(c, true))}{now && day === today && mins(timeLabel(now, tz)) >= 480 && mins(timeLabel(now, tz)) <= 1200 && <div className="nowline" style={{ top: (mins(timeLabel(now, tz)) - 480) / 60 * 64 }} />}</div></div>{!dayChips.length && <div className="empty-day">Nothing booked for {dayTitle}.</div>}</div>{unscheduled}</div></>
  } else if (view === 'profile' && file) {
    const q = data.questionnaire!, progress = answerCount(q.answers)
    content = <><article className="card ph"><div><button className="btn secondary" onClick={() => go('board')}>← Board</button><div className="who">{String(q.answers.legal_name || `${file.firstName} ${file.lastName}`)}</div><div className="muted">{file.city}, {file.state} {file.zip} · {file.source} · {file.stage}</div><div className="tags"><span className={`tag ${file.ownerName ? 'g' : 'w'}`}>Owner: {file.ownerName || 'Unassigned'}</span><span className="tag">{file.appointmentLabel ? `Appt ${file.appointmentLabel}` : 'Not booked'}</span><span className="tag">Docs {file.docsPresent}/8</span><span className={`tag ${file.extractionLabel === 'verified' ? 'g' : 'w'}`}>Extraction {file.extractionLabel}</span><span className={`tag ${file.creditOnFile ? 'g' : 'w'}`}>{file.creditOnFile ? `Credit ${file.creditLabel}` : 'Credit not on file'}</span></div></div><div className="actions">{file.canAssign && <button className="btn" onClick={() => openModal('assign')}>Assign closer</button>}<button className="btn secondary" disabled={!file.canBook} onClick={() => openModal('book')}>{file.appointmentLabel ? 'Reschedule' : 'Book'}</button><button className="btn secondary" onClick={() => void draftRequest()}>Request missing docs</button><button className="btn secondary" onClick={() => void copyContact()}>Call</button></div></article><div className="g2"><div className="card block"><h3>Finance agreement</h3><div className="math">{file.finance.map(c => <Cell key={c.label} value={c} />)}</div></div><div className="card block"><h3>Solar / intake</h3><div className="math">{file.solar.map(c => <Cell key={c.label} value={c} />)}</div></div></div><div className="card block" style={{ marginBottom: 14 }}><h3>Documents</h3><p className="muted" style={{ margin: '-4px 0 12px' }}>Click Quick look to open the page. Live desk will load the real file; this prototype shows the extract when a document exists.</p><div className="docs">{file.docs.map(d => <div key={d.key} className="doc"><div className="doc-top"><span>{(t.tiles as Record<string, string>)[d.key] ?? d.label}</span><div className="doc-acts"><button className="lookbtn" onClick={() => setLook(d)}>Quick look</button><button className="lookbtn" disabled={busy || !file.canUpload} onClick={() => void upload(d)}>Upload</button></div></div><div className={`st ${d.state === 'missing' || d.state === 'failed' ? 'm' : 'g'}`}>{d.state}</div></div>)}</div></div><div className="g2"><div className="card block"><h3>Intake answers · verbatim</h3><div className="qa">{file.intake.map((a, i) => <div key={i}><b>{a.question}</b>{a.answer}</div>)}</div><button className="qtab" style={{ margin: '14px 0 0' }} onClick={() => router.push(`/clients/${file.id}/questionnaire`)}><div><b>Client&apos;s Questionnaire Form</b><div className="muted">{q.done ? 'All 42 answered' : `${progress} of 42 answered · finish on the booking call`}</div></div><div className={`mark tag ${q.done ? 'g' : 'w'}`}>{q.done ? '✓ Completed' : 'In progress'}</div></button></div><div className="card block prose"><h3>Redline · internal posture</h3><p className="muted">State and federal consumer rights for this file. Not a homeowner lecture.</p><p><b>{file.state || 'State'} consumer rights</b></p><ul>{file.redline.state.map((t, i) => <li key={i}>{t}</li>)}</ul><p><b>Federal consumer rights</b></p><ul>{file.redline.federal.map((t, i) => <li key={i}>{t}</li>)}</ul></div></div><div className="card block prose" style={{ marginTop: 14 }}><h3>Closer brief {file.brief?.approved ? '· approved' : ''}</h3>{file.callSheet && <><p><b>Master call sheet</b> · {file.callSheet.disclaimer}</p><p><b>Opening — then stop.</b> {file.callSheet.opening}</p>{file.callSheet.sections.map(section => <div key={section.title}><p><b>{section.title}</b></p><p>{section.say}</p><ul>{section.facts.map(fact => <li key={fact.label}><b>{fact.label}:</b> {fact.value}</li>)}</ul></div>)}<p><b>The ask — then stop.</b> {file.callSheet.ask}</p><p><b>Documents</b></p><ul>{file.callSheet.documents.map(doc => <li key={doc.item}><b>{doc.item}:</b> {doc.note}</li>)}</ul></>}<p>{file.brief?.body || 'No closer brief on file.'}</p><p>Surprise numbers: {file.brief?.sur.join('; ') || 'Not on file'}.</p><p>First ask: {file.brief?.ask || 'Review the available documents'}.</p><p>Suggested open: {file.brief?.open || 'Not on file'}.</p><div className="actions" style={{ marginTop: 10 }}><a className="btn" href={`/api/clients/${file.id}/closer-packet?kind=review`}>Case review PDF</a><a className="btn secondary" href={`/api/clients/${file.id}/closer-packet?kind=pitch`}>Closer pitch PDF</a><button className="btn" disabled={busy || !file.brief} onClick={() => void run(() => approveBriefAction({ clientId: file.id, briefId: file.brief!.id }), 'Brief approved')}>{file.brief?.approved ? 'Approved' : 'Approve brief'}</button><button className="btn secondary" disabled={busy || !file.brief} onClick={() => { const body = window.prompt('Edit closer brief', file.brief!.body); if (body?.trim()) void run(() => editBriefAction({ clientId: file.id, briefId: file.brief!.id, body }), 'Brief updated · needs approve') }}>Edit</button></div></div></>
  ...
  return <div className="final-desk" onClick={() => menu && setMenu(null)}><div className="shell"><aside className="rail"><div className="brand"><small>SCS operations</small><strong>ProdigyFlo</strong></div><nav className="nav">{['board','clients','queue'].map(v => <button key={v} className={view === v ? 'on' : ''} onClick={() => go(v)}>{v === 'queue' ? `${t.titles.queue} · ${data.queueCount}` : t.titles[v as DeskView]}</button>)}<div className="sec">{t.nav.more}</div>{['engine','documents','submissions','users'].filter(v => isAdmin || !['engine','users'].includes(v)).map(v => <button key={v} className={view === v ? 'on' : ''} onClick={() => go(v)}>{t.titles[v as DeskView]}</button>)}</nav><div className="rail-foot"><b>{data.user.name}</b>{isAdmin ? 'Super Admin' : 'Closer'}<br />{t.inviteOnly}<br /><button className="out" onClick={() => void signOut({ callbackUrl: '/login' })}>{t.signOut}</button></div></aside><div className="col"><header className="top"><div><h1>{t.titles[view]}</h1><div className="muted">{now ? `${now.toLocaleDateString(locale === 'es' ? 'es-US' : 'en-US', { timeZone: viewerTz, weekday: 'long', month: 'short', day: 'numeric', year: 'numeric' })} · 🕐 ${now.toLocaleTimeString(locale === 'es' ? 'es-US' : 'en-US', { timeZone: viewerTz, hour: 'numeric', minute: '2-digit', timeZoneName: 'short' })}` : ''}</div></div><div style={{ display: 'flex', alignItems: 'center', gap: 12 }}><LocaleSwitch /><div className={`alarm${soon ? ' hot' : ''}`}><span className="bell">⏰</span><span>{soon && now ? `${soon.firstName} ${soon.lastName} · in ${Math.max(1, Math.ceil((new Date(soon.startsAt).getTime() - now.getTime()) / 60000))} min` : 'No call in the next 30 min'}</span></div><div className="muted">Not a law firm · review only</div></div></header><main className="main">{content}</main></div></div>
    ...
    {look && <DocumentQuicklook key={look.key} tile={look} allFiles={file?.availableFiles ?? []} clientLabel={`${file?.firstName} ${file?.lastName} · ${file?.city}, ${file?.state} ${file?.zip}`} firstName={file?.firstName ?? ''} onClose={() => setLook(null)} onRequest={() => void draftRequest(look)} />}
