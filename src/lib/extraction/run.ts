...
export async function runExtraction(documentId: string): Promise<ExtractionRunResult> {
  ...
  const imported = await db.externalDocumentImport.findFirst({where:{clientDocumentId:documentId},select:{id:true,sourceLeadId:true,sourceAnalysis:true,sourceDocumentType:true}})
  if (imported?.sourceLeadId && imported.sourceDocumentType !== 'public_record_summary') {
